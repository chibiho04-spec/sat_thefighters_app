# 売上表エクスポート：外注費明細＋整形出力 設計仕様

作成日：2026-06-18 / 対象：`案件管理アプリ/index.html`（単一HTMLファイル）

> **【2026-06-18 方針変更】出力形式を Excel(.xlsx) → PDF に変更。** 経理確認の結果Excelデータである必要がなく、「表をPDFでダウンロード」に決定。外注費明細・列構成・金額計算・rowspan結合・￥表示はそのまま、出力だけ「新しい印刷ウィンドウに表＋A4横向きCSSを書き出し、印刷ダイアログから PDF として保存」（請求書PDF `executePrint` と同方式）に置換。実装関数は `_openSalesPrint` / ボタンは「📥 PDF」。以下「.xls/.xlsx」の記述は経緯として残すが、最終成果物はPDF。
